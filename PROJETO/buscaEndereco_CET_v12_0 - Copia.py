...
def enriquecer_candidatos_geoserver(df_candidatos: pd.DataFrame) -> pd.DataFrame:
    ...
    GEOSERVER_LAYERS = [
        "vwGets", "vwDets", "mdcSubPrefeitura",
        ...
        for tentativa in range(MAX_TENTATIVAS_REQUISICAO):
            try:
                ...
                for f in data.get("features", []):
                    ...
                    if layer.endswith("gets"): out["GET"] = props.get("sigla")
                    if layer.endswith("dets"): out["DET"] = props.get("sigla")
                    if layer.endswith("mdcSubPrefeitura"): out["SUB"] = props.get("sigla2")
                    if layer.endswith("mdcDistrito"): out["Distrito_Nome"] = props.get("Nome_distr")
                    if layer.endswith("regiao5"): out["Regiao_Nome"] = props.get("nome")
                    if layer.endswith("ClassVia"): out["Classificacao"] = props.get("Classificacao")
                return out
                
            except (requests.RequestException, ValueError) as e:
                if tentativa == MAX_TENTATIVAS_REQUISICAO - 1:
                    logging.debug(f"❌ GEOSERVER falhou após {MAX_TENTATIVAS_REQUISICAO} tentativas")
                    return {}
                else:
                    time.sleep(1 * (tentativa + 1))
            except Exception as e:
